...
class LdmExtractor(nn.Module):
    ...
    # }
    LDM_CONFIGS = {
        "SSD-1B": ("/home/zhanghaotian/DiffCut/hfckpts/SSD-1B", "XL"),
        "SSD-vega": ("/home/zhanghaotian/DiffCut/hfckpts/Segmind-Vega", "XL"),
        "SD1.4": ("/home/zhanghaotian/DiffCut/hfckpts/stable-diffusion-v1-4", None)
    }
    ...
    def _forward(self,
                ...
                img_size: int = None,
                prompts: List[str] = None,
                max_batch_size: int = 4  # New parameter to limit mini-batch size
                ):

        if img_size is not None:
            # Resize images and set up prompts
            images = F.interpolate(img, size=(img_size, img_size), mode='bilinear')
        else:
            images = img
        ...
        # Prepare a container for accumulating outputs
        all_features = []
        all_attn_maps = []

        # Process in mini-batches
        for i in range(0, batch_size, max_batch_size):
            # Select a mini-batch of images
            batch_images = images[i:i + max_batch_size]
            current_batch_size = batch_images.shape[0]
            prompt_embeds_ = prompt_embeds[i:i + max_batch_size]
            added_cond_kwargs_ = {k: v[i:i + max_batch_size] for k, v in added_cond_kwargs.items()}

            # Encode latent images for the current mini-batch
            latent_images = self.encode_to_latent(batch_images)
            
            # Generate noise for the mini-batch
            noise = torch.randn(1, 4, img_size // 8, img_size // 8, generator=rng, device=self.device)
            noise = noise.expand(current_batch_size, -1, -1, -1)
            
            # Set up the time tensor for the scheduler
            t = torch.tensor([step], device=self.device).expand(current_batch_size)
            
            # Add noise to the latent representations
            noisy_latent_batch = self.pipe.scheduler.add_noise(latent_images, noise, t)
                
            if self.do_classifier_free_guidance(guidance_scale):
                noisy_latent_batch = torch.cat([noisy_latent_batch] * 2)
            t = t.repeat(noisy_latent_batch.shape[0] // current_batch_size)

            # Register hooks to capture the features and attention maps
            hook_handles = self.register_hooks()
                
            # Run the UNet for the mini-batch
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                with torch.no_grad():
                    scaled_input = self.pipe.scheduler.scale_model_input(noisy_latent_batch, t)
                    # The outputs are captured via hooks, not directly returned
                    self.unet(scaled_input, t, encoder_hidden_states=prompt_embeds_, added_cond_kwargs=added_cond_kwargs_)
            
            # Remove hooks and store captured outputs
            for hook_handle in hook_handles:
                hook_handle.remove()

            # Append the outputs for this mini-batch (detach and optionally move to CPU)
            all_features.append(self._features.detach().cpu())
            all_attn_maps.append(self._attn_map.detach().cpu())

        # Concatenate outputs from all mini-batches along the batch dimension
        combined_features = torch.cat(all_features, dim=0)
        combined_attn_maps = torch.cat(all_attn_maps, dim=0)

        return combined_features, combined_attn_maps
