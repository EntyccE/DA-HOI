...
class CLIP(nn.Module):
    ...
    def visual_prediction_forward_convnext(self, x):
        batch, num_query, channel = x.shape # 1,1,1536
    ...
    def extract_features_convnext(self, x):
        ...
        # # Flatten patches and add positional embeddings here if needed
...
class open_vocabulary:
    ...
    def prepare_class_names_from_metadata(self, metadata, train_metadata):
        ...
        # get text classifier
        try:
            class_names = split_labels(metadata) # it includes both thing and stuff
            train_class_names = split_labels(train_metadata)
        except:
            try:
