...
HICO_TRAIN_ATTN = "./resources/train_attention_maps.h5"
HICO_VAL_ATTN = "./resources/test_attention_maps.h5"

HICO_TRAIN_MASK = "./resources/train2015_mask.h5"
HICO_VAL_MASK = "./resources/test2015_mask.h5"
...
class HICO(CocoDetection):
    def __init__(
        ...
        object_to_related_hois, action_to_related_hois = prepare_related_hois(
            hico_unseen_index[zero_shot_type], ignore_non_interaction
        )
    ...
    def _unify_noun_idx(self):
        ...
        for key, info in self.mask_dict.items():
            ...
            if self.mask_locate_type == "merged":
                self.mask_dict[key]["mask"] = torch.from_numpy(
                    info["original_mask"]
                ).int()
            else:
    ...
    with h5py.File(hdf5_file, "r") as f:
        with h5py.File(attn_file, "r") as f1:
            ...
            for image_key in f.keys():
                ...
                image_data = {}
                image_data["mask"] = group["mask"][()]  # Semantic mask.
                image_data["original_mask"] = group["original_mask"][
                    ()
                ]  # Refined original mask.
                # image_data["diffusion_feature"] = group["diffusion_feature"][()]  # Diffusion feature vector.
                ...
                attn_t = F.interpolate(
                    attn_t, size=(h1, w1), mode="bilinear", align_corners=False
                )
                ...
                if any(len(noun) > 25 for noun in original_nouns):
                    ...
                    valid_indices = [
                        i for i, noun in enumerate(original_nouns) if len(noun) <= 25
                    ]
...
def load_hico_json(
    ...
    if repeat_factor_sampling:
        repeat_factors = repeat_factors_from_category_frequency(
            dataset_dicts, repeat_thresh=0.003
        )
...
def prepare_related_hois(zero_shot_interaction_ids, ignore_non_interaction):
    ...
    for x in HICO_INTERACTIONS:
        ...
        if hoi_id in zero_shot_interaction_ids or (
            hoi_id in NON_INTERACTION_IDS and ignore_non_interaction
        ):
            continue
